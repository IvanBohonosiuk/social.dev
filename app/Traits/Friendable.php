...
trait Friendable
{
	...
	public function add_friend($user_requested_id)
	{
		...
		if ($this->is_friends_with($user_requested_id) === 1) {
			return 'already frinds';
		}

		if ($this->has_pending_friend_request_send_to($user_requested_id) === 1) {
	...
	public function pending_friend_requests_send()
	{
		$users = [];
	...
	public function pending_friend_requests_send_ids()
	{
		return collect($this->pending_friend_requests_send())->pluck('id')->toArray();
	}
	...
	public function has_pending_friend_request_send_to($user_id)
	{
		if (in_array($user_id, $this->pending_friend_requests_send_ids())) {
			return 1;
		} else {
			return 0;
		}
